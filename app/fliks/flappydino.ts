...
var fgSVG = \`
            <svg id="fg-svg" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns="http://www.w3.org/2000/svg"
                ...
                <use xlink:href="#a" transform="translate(0 -3.7151e-8)" height="100%" width="100%" y="0" x="0" />
            </svg>
        \`;
...
var bonePtr = 0;
var boneStart = 400;
var flyGap = 200; // Increased from 180 to 200 for easier gameplay
var boneGap = 280;
var bones = [
...
function resize() {
  ...
  // Calculate scale to fit height (standard mobile view style) or width?
  // Let's scale to fit the smallest dimension so it's always visible, but fill screen
  // Actually, Flappy Bird usually has a fixed width and extends height.
  // Let's try to maintain aspect ratio of game logic (360 width)
  
  // Strategy: Scale so that 360px width takes up a reasonable amount of screen width on desktop,
  // or full width on mobile.
  
  var targetWidth = 360;
  var targetHeight = 640;
  
  var scaleX = window.innerWidth / targetWidth;
  var scaleY = window.innerHeight / targetHeight;
  
  // Use the smaller scale to fit everything
  scaleFactor = Math.min(scaleX, scaleY);
  
  // If on desktop (wide screen), maybe we don't want it HUGE. 
  // But user said "fill the full screen".
  // Let's just use scaleFactor.
  
  // Center the game play area
  ctx.imageSmoothingEnabled = false; // Crisp pixels if desired
}
...
function checkCollision() {
  ...
  // dino hit the ground - account for variable height?
  // The logic assumes 640 height. We need to ensure ground collision works relative to logical height.
  // Using 640 as logical height bottom.
  
  if (dinoPos_Y + hitboxHeight >= 640 - fg.height) {
    velocity = 0;
    dinoPos_Y = 640 - fg.height - hitboxHeight;
    setGameOver();
  }
...
function render() {
  ...
  // Clear entire canvas
  ctx.fillStyle = "#d7eef4";
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  ctx.save();
  
  // Center the game logic (360x640) on screen
  var logicWidth = 360;
  var logicHeight = 640;
  
  var offsetX = (canvas.width - logicWidth * scaleFactor) / 2;
  var offsetY = (canvas.height - logicHeight * scaleFactor) / 2;
  
  // Fill bottom gap with ground color to ensure floor reaches bottom
  if (offsetY + logicHeight * scaleFactor < canvas.height) {
    ctx.fillStyle = "#ffb380";
    // Draw a rectangle from the bottom of the game area to the bottom of the canvas
    // Add a small overlap (1px) to prevent gaps
    var gameBottom = offsetY + logicHeight * scaleFactor;
    ctx.fillRect(0, gameBottom - 1, canvas.width, canvas.height - gameBottom + 1);
  }
  
  ctx.translate(offsetX, offsetY);
  ctx.scale(scaleFactor, scaleFactor);

  // Clip to game area width, but allow top to extend (negative Y), clip bottom at 640
  ctx.beginPath();
  ctx.rect(0, -10000, 360, 10000 + 640);
  ctx.clip();

  // Draw background (360x640)
  ...
  // We need to tile the foreground if the screen is wider than 360?
  // But we are scaling the whole context, so the 360-width foreground will just be scaled up.
  ctx.drawImage(fg, fgPos_X, 640 - fg.height);
  ctx.drawImage(fg, fgPos_X + fg.width, 640 - fg.height);
  
  // Draw extra ground to cover gaps if any logic issues (though with scaling, should be fine)
