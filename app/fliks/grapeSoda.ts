// Full standalone HTML for Grape Soda
const getGrapeSodaHTML = () => {
  return `<!DOCTYPE html>
<html lang="en">
...
function init() {
  ...
  document.addEventListener('touchmove', e => {
    if (e.touches.length > 0) {
      let ratio = window.innerHeight / window.innerWidth;
      uniforms.u_mouse.value.x = (e.touches[0].pageX - window.innerWidth / 2) / window.innerWidth / ratio;
      uniforms.u_mouse.value.y = (e.touches[0].pageY - window.innerHeight / 2) / window.innerHeight * -1;
      e.preventDefault();
    }
  });
}
...
export default getGrapeSodaHTML();
