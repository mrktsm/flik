// Full standalone HTML for Letterfall
const getLetterfallHTML = () => {
  return `<!DOCTYPE html>
<html lang="en">
...
window.addEventListener("error", (event) => {
  window.ReactNativeWebView?.postMessage(
    JSON.stringify({ type: "error", message: event.message || "Unknown error" })
  );
});
...
export default getLetterfallHTML();
