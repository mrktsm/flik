// Full standalone HTML for Mini GSAP Game - A Little to the Right
const getMiniGsapGameHTML = () => {
  return `<!DOCTYPE html>
<html lang="en">

  <head>
    <meta charset="UTF-8">
    <title>Mini GSAP Game - A Little to the Right</title>
...
.win #mouth {
  /* Path will be updated via JavaScript */
}
.win #eyes {
  /* Path will be updated via JavaScript */
}
...
// Set up Timeline for wallpaper triangles (optimized - animate fewer paths)
let triangleTL = gsap.timeline({
  paused: true,
  repeat: -1,
  yoyo: true,
  ease: "power1.inOut" // Simpler easing for better performance
});
...
// Cache elements and animations for better performance
let shadowAnim, innerShadowAnim, glareAnim;
let lastRotation = null;

function adjustPicture(rotation, duration = 0) {
  /* 
  Updates shadows and glare when the picture rotates.
  Optimized to avoid creating new animations on every frame.
  */
  // Skip if rotation hasn't changed significantly (during drag)
  if (duration === 0 && lastRotation !== null && Math.abs(rotation - lastRotation) < 0.1) {
    return;
  }
  ...
  if (duration === 0) {
    // During drag, use set for immediate updates (no animation)
    gsap.set("#shadow", { x: outerShadowX, y: outerShadowY });
    gsap.set("#inner-shadow", { x: innerShadowX, y: innerShadowY });
    gsap.set("#glare-group", { 
      rotate: -1 * rotation,
      transformOrigin: "center center"
    });
  } else {
    // For smooth transitions, use to
    if (shadowAnim) shadowAnim.kill();
    if (innerShadowAnim) innerShadowAnim.kill();
    if (glareAnim) glareAnim.kill();
    
    shadowAnim = gsap.to("#shadow", {
      x: outerShadowX,
      y: outerShadowY,
      duration,
      ease: "power2.out"
    ...
    innerShadowAnim = gsap.to("#inner-shadow", {
      x: innerShadowX,
      y: innerShadowY,
      duration,
      ease: "power2.out"
...
function handleMoveEnd(rotation) {
  ...
  */
  ...
  if (rotation > -1 && rotation < 1) {
    ...
    // Make the cat smile!
    if (mouth) {
      mouth.setAttribute("d", "M388.5 292C376 320.5 341 307.5 336 266C342 300 311 330 288 297");
    }
    if (eyes) {
      eyes.setAttribute("d", "M208 216C229 208 247 196 279 218.5M361.5 218.5C379.5 208 407 187 441 210.5");
    }
  } else {
    ...
    if (mouth) {
      mouth.setAttribute("d", "M374 289.5C360.5 288.5 342.5 288.5 336 266C338.53 287 314 287 302 291");
    }
    if (eyes) {
      eyes.setAttribute("d", "M208 216C226 212 254 212 279 218.5M361.5 218.5C379.5 216 411.5 210.5 441 210.5");
    }
  }
}
...
function init() {
  ...
  // Only animate visible paths for better performance
  const wallpaperPaths = document.querySelectorAll("#wallpaper path");
  wallpaperPaths.forEach(path => {
    gsap.set(path, { transformOrigin: "center" });
  });
  ...
  */
  ...
  Draggable.create("#picture-frame", {
    ...
    onDrag: function () {
      ...
      // Only hide arrow once
      if (!arrowHidden && arrow) {
...
export default getMiniGsapGameHTML();
